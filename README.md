...
- ранняя остановка, сохранение чекпоинтов и опциональное логирование в ClearML.

## Результат последнего обучения

Запуск был остановлен early stopping'ом на 34-й эпохе из запланированных 100. Лучший чекпоинт соответствует 14-й эпохе.

| Показатель | Значение |
| --- | ---: |
| Лучшая validation loss | 3.8797 |
| Эпоха лучшего чекпоинта | 14 |
| Train loss на финальной эпохе | 2.9984 |
| Validation loss на финальной эпохе | 3.9192 |
| BLEU на финальной эпохе | 0.1655 |
| METEOR на финальной эпохе | 0.3774 |
| Размер словаря | 4 697 |
...
Модель уже улавливает основные объекты и простые сцены, но заметны повторы слов и `<UNK>` для редких слов. Это ожидаемое ограничение текущего словаря и greedy-декодирования.
...
- NVIDIA GPU с CUDA — рекомендуется. Если CUDA недоступна, загрузчик конфигурации автоматически переключит устройство на CPU, однако обучение будет существенно медленнее;
- доступ к интернету при первом запуске: для установки зависимостей, загрузки весов ResNet50 и, при необходимости, датасета/ресурса NLTK WordNet.
...
Зависимости, включая PyTorch и torchvision с CUDA 12.6, описаны в [`pyproject.toml`](pyproject.toml).
...
Каждый чекпоинт содержит веса модели и оптимизатора, номер эпохи, лучшую validation loss, словарь `word2idx`/`idx2word` и актуальную конфигурацию. Это позволяет восстановить модель с тем же словарём для отдельного инференса.

В конфигурации включены `seed: 42` и детерминированный режим cuDNN. Полная битовая воспроизводимость между разными версиями CUDA, драйверов или PyTorch всё же не гарантируется.
